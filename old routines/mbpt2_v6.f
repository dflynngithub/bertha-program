...
C
      ...
      COMPLEX*16 B(NUMO*MBS,8),SB(MB2,NUMO*NUMV,4)
...
C
      CALL CPU_TIME(TBEG)
C
CC     IMPORT MBPT1 PAIR RESULTS FOR E1(ab)
C      OPEN(UNIT=8,FILE=TRIM(OUTFL)//'_MBPT1.dat',STATUS='UNKNOWN')
C      REWIND(UNIT=8)
C      DO IOCCA=1,NUMO
C        DO IOCCB=1,NUMO
C          READ(8, *) Q1,Q2,Q3,(EAB2(IOCCA,IOCCB,N),N=1,3)
C        ENDDO
C      ENDDO
C      CLOSE(UNIT=8)
C
