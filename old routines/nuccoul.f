...
C       PROTON HAS A VERY SMALL RADIUS (IN FM)
        RFM = PRAD
      ELSE
C       EMPIRICAL FORMULA FOR NUCLEAR RMS RADIUS (IN FM)
...
C     CONVERT RESULT TO ATOMIC UNITS
      RNUC(IZ) = RFM/CFM
C
C     IF NFT=0 USE THE POINT-NUCLEUS MODEL
      IF(NFT.EQ.0) THEN
        RNUC(IZ) = 0.0D0
        RSQBIG   = 1.0D0
        GOTO 20
      ENDIF
C
C     IF NFT=1 THE GAUSSIAN MODEL IS THE ONLY OPTION
      IF(NFT.EQ.1) THEN
        FNUC(IZ,1) = 1.0D0
...
49    FORMAT(2X,A,14X,A,14X,A,14X,A,14X,A)
52    FORMAT(2X,'<r^',I2,'>',4(2X,ES18.10))
      WRITE(6, *) ''
      WRITE(7, *) ''
      WRITE(6,49) 'Moment','Unifrm',' Fermi','GaussN','Gauss0'
      WRITE(7,49) 'Moment','Unifrm',' Fermi','GaussN','Gauss0'
      WRITE(6, *) REPEAT('-',87)
      WRITE(7, *) REPEAT('-',87)
      CF = 1.0D0/(CFM**2)
      DO I=-2,8
        WRITE(6,52) I,RU(I)*CF,RF(I)*CF,RG(I)*CF,R0(I)*CF
        WRITE(7,52) I,RU(I)*CF,RF(I)*CF,RG(I)*CF,R0(I)*CF
        CF = CF*CFM
      ENDDO
      WRITE(6, *) REPEAT('-',87)
      WRITE(7, *) REPEAT('-',87)
