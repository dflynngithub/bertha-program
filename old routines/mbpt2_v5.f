C NOTE: THIS IS A WORKING ROUTINE, BUT IS WRITTEN TO TAKE ADVANTAGE OF
C       R-INTEGRAL SYMMETRIES IN C<->D, AT THE EXPENSE OF LOOP STRUCTURE
C       SHORTCUTS. I HAVE FOUND THAT R-INTEGRALS ARE CHEAP ENOUGH THAT
C       IS OMPTIMAL TO INCLUDE A<->B SYMMETRY RELATIONS BUT NOT C<->D.
C       THERE IS ALSO SOME PROTOTYPE OPENMP CODE IN HERE (COMMENTED OUT).
C
...
C
C**********************************************************************C
C     SECOND CONTRACTION: (THIS REQUIRES MOST COMP. TIME)      ~       C
...
CC$OMP&  SHARED(SB,B1,C)
CC$OMP&  PRIVATE(IS,MSB,KBAS,MKB)
C
...
C     INITIALISE AN OPENMP PARALLEL REGION
CC$OMP PARALLEL DO COLLAPSE(2)
CC$OMP&  SHARED(SB,B2,C)
CC$OMP&  PRIVATE(IS,MSB,LBAS,MLB)
C
C     LOOP OVER OCCUPIED STATES IOCCB AND VIRTUAL STATES IVRTS
      DO IOCCB=1,NUMO
        DO IVRTS=1,NUMV
