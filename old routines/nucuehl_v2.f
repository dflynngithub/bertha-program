      SUBROUTINE NUCUEHL(IZ,NFT,IWRT,AR,BF,GF,RSBG)
...
C
      ...
      COMMON/TEMP/XUEH(MCT,MFT),FUEH(MCT,MFT)
C
...
C
C**********************************************************************C
C     NUMERICAL EVALUATION OF UEHLING POTENTIAL V(R).                  C
C**********************************************************************C
C
...
C     GEOMETRIC PARAMETER SEARCH DETAILS
      ALPH = 2.00D-5/(RNUC(IZ)**2)
      BETA = 1.60D+0
      GAMA = 0.21D+0
c      ALPH = 1.50D-5
c      BETA = 1.60D+0
c      GAMA = 0.00D+0
c      gama = 0.707683d0*dexp(-0.564865d0*alph*1.0d5)
c     gama = 0.15d+0
C
...
C     INITIALISE BEST-FIT R-SQUARED AND INDEX FOR ALPHA
      ...
      IGBG = 0
cC
cC     ITERATE OVER GAMMA SEARCH
c      DO IG=0,NGF
cC
C       TRIAL GAMMA PARAMETER
c        GF = GAMA + 0.25D+0*DFLOAT(IG)/DFLOAT(NGF)
        gf = gama
C
...
C         R-SQUARED VALUE AND BEST-FIT CHECK
          ...
          ENDIF
C
C          IF(SRES/STOT.LT.3.0D-9) THEN
C            WRITE(*,*) 'EARLY FINISH'
C            GOTO 15
C          ENDIF
C
        ENDDO
...
C     BEST-FIT VALUES FOR ALPHA, BETA AND GAMMA
      ...
      BF = BETA
      GF = GAMA + 0.25D+0*DFLOAT(IGBG)/DFLOAT(NGF)
      gama = 0.15d+0
c     gama = 0.707683d0*dexp(-0.564865d0*af*1.0d5)
      gf   = gama
      AF = af/(RNUC(IZ)**2)
C
C     ALPHA RE-SCALED
      AR = AF*RNUC(IZ)*RNUC(IZ)
C
C     STORE GEOMETRIC SET OF PARAMETERS IN XUEH
      XI = AF
      DO IFT=1,NFT
      ...
      ENDDO
C
      WRITE(*,*) RSBG,AF*RNUC(IZ)*RNUC(IZ),BF,GF
      WRITE(*,*) IABG,0,IGBG
...
38    FORMAT(1X,A,I2,A,F10.5,A,F8.5,A)
      ...
      WRITE(6,37) 'α = ',AR,' ξ0,  β = ',BF,',  γ = ',GF
      WRITE(7,37) 'α = ',AR,' ξ0,  β = ',BF,',  γ = ',GF
      WRITE(6, *) REPEAT('-',45)
      WRITE(7, *) REPEAT('-',45)
...
C     UEHLING DIFFERENCES
      XOUT   = 'Uehling-difference'//TRIM(ATRM)
