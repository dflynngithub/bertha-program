C     THIS VERSION IS FROM AUGUST 2019 AND APPEARED NOT TO BE WORKING.
C     I ALTERED THE LOOP ORDERS IN SOME OF THE CONTRACTIONS, AND THIS
C     CAUSED ISSUES.
C
...
C
CCC      NUMBER OF NUCLEAR CENTRES INVOLVED IN THIS OVERLAP
CCC      MCNT = NCNTRS(ICNTA,ICNTB,ICNTC,ICNTD)
CCC      IF(MCNT.NE.1.AND.ICNTA.NE.1) GOTO 5001
C
C**********************************************************************C
...
C**********************************************************************C
C
CC     ATOM-CENTRED SELECTION RULES
C      IF(MCNT.EQ.1) THEN
CC
CC       LQN PAIR PARITY (0 IF EVEN, 1 IF ODD)
C        IPARAB = MOD(LQN(1)+LQN(2),2)
C        IPARCD = MOD(LQN(3)+LQN(4),2)
CC
CC       LQN PAIR PARITY SELECTION RULE
C        IF(IPARAB.NE.IPARCD) THEN
C          GOTO 6001
C        ENDIF
CC
CC       JQN TRIANGLE RULE CHECK FOR MULTIPOLE EXPANSION (ATOM-CENTRED)
C        NUI = MAX0(IABS(JQN(1)-JQN(2))/2,IABS(JQN(3)-JQN(4))/2)
C        NUF = MIN0(    (JQN(1)+JQN(2))/2,    (JQN(3)+JQN(4))/2)
C        IF(NUI.GT.NUF) THEN
C          GOTO 6001
C        ENDIF
CC
CC       ADDITIONAL LQN SELECTION RULE PARITY ANALYSIS
C        ISELK = 0
C        DO NU=NUI,NUF
CC
CC         A AND B: LQN(1)+LQN(2)+NU EVEN OR ODD (0 IF EVEN, 1 IF ODD)
C          IPARAB = MOD(LQN(1)+LQN(2)+NU,2)
CC
CC         C AND D: LQN(3)+LQN(4)+NU EVEN OR ODD (0 IF EVEN, 1 IF ODD)
C          IPARCD = MOD(LQN(3)+LQN(4)+NU,2)
CC
CC         CASE 1: LQNA+LQNB+NU AND LQNC+LQND+NU ARE BOTH ODD (UNLESS NU=0)
C          IF(IPARAB.EQ.1.AND.IPARCD.EQ.1.AND.NU.NE.0) ISELK = 1
CC
CC         CASE 2: LQNA+LQNB+NU AND LQNC+LQND+NU ARE BOTH EVEN
C          IF(IPARAB.EQ.0.AND.IPARCD.EQ.0) ISELK = 1
CC
C        ENDDO
...
C      IF(ISYM.EQ.1) THEN
C        ISELM = 0
C        IF(MQN(1).EQ.MQN(2).AND.MQN(3).EQ.MQN(4)) ISELM = 1
C        IF(MQN(1).EQ.MQN(3).AND.MQN(2).EQ.MQN(4)) ISELM = 1
C        IF(MQN(1).EQ.MQN(4).AND.MQN(2).EQ.MQN(3)) ISELM = 1
C        IF(ISELM.EQ.0) GOTO 7001
C      ENDIF
CC
CC     ATOM-CENTRED SELECTION RULES (ONLY APPLIES IF RACAH1 SWITCHED OFF)
C      IF(MCNT.EQ.1) THEN
C        ISELM = 0
C        DO ISGN1=1,2
C          DO ISGN2=1,2
...
C                MMJC = MQN(3)*((-1)**ISGN3)C
C                MMJD = MQN(4)*((-1)**ISGN4)
C                IF(MMJA-MMJB.EQ.MMJD-MMJC) ISELM = 1
C              ENDDO
C            ENDDO
C          ENDDO
C        ENDDO
