...
C -------------------------------------------------------------------- C
C  DFNOTE: DO I NEED TO INCORPORATE ITN(2) INTO THIS?                  C
C**********************************************************************C
...
C
      ...
      CHARACTER*8 SHAPE
C
      DIMENSION EXPT(MBS,4),XYZ(3,4),KQN(4),LQN(4),MQN(4),NBAS(4),ITN(2)
...
C      DO IT=1,2
C        IF(ITN(IT).NE.2.AND.ITN(IT).NE.3) THEN
C          WRITE(6, *) 'In BII: illegal component overlaps in ITN.'
C          WRITE(7, *) 'In BII: illegal component overlaps in ITN.'
C          STOP
C        ENDIF
C      ENDDO
C
...
C
        CALL CPU_TIME(T1)
C
C       THIS HAS TO DO WITH SCREENING PROCEDURE -- FIX LATER
        MAXN = NBAS(3)*NBAS(4)
C
...
C         COPY THIS BATCH OF INTEGRALS TO A SAVED LIST
          ...
          DO KBAS=1,NBAS(3)
            DO LBAS=1,NBAS(4)
              M = M+1
              DO IABCDFL=1,NTUVABCD
                IAD = IADRTT + (M-1)*NTUVABCD + IABCDFL
