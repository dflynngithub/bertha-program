...
C     COMMON TERMS TO ALL OPTIONS
      ...
      ENDDO
C
C     CALCULATE FROM SCRATCH (ONE-TIME USE)
      IF(.NOT.RCFILE) THEN
C
        CALL CPU_TIME(T1)
C
...
C
      ENDIF
C
C     CALCULATE FROM SCRATCH AND WRITE TO LOCAL RC(AB|CD) FILE
      IF(RCFILE.AND.IRIJ(IBAS,JBAS).EQ.1) THEN
C
        CALL CPU_TIME(T1)
C
C       GAUSSIAN OVERLAP CENTRE
...
C
C       RECORD TIME SPENT WRITING R-INTEGRALS
        CALL CPU_TIME(TA)
        DO M=1,MAXCD
          DO IABCDFL=1,NTUVABCDFL
...
C       STARTING ADDRESS FOR SAVED R(AB|CD) INTEGRALS
        IADRTT = (IJ-1)*NBAS(3)*NBAS(4)*NTUVABCDFL
C
C       READ THE FULL LIST OF R(AB|CD)
C        NREC = ISTOP-ISTRT
C        READ(UNIT=10,REC=NREC) (VLLLL(I),I=ISTRT,ISTOP)
C
...
C         CONTRIBUTIONS TO Re{G(AB|--)} FROM EACH Re{E(CD|--)} ADDRESS
          ...
          IF(ICDR11(ICD).EQ.1) THEN
            DO N=1,MAXN
              IAD = (IJ-1)*NBAS(3)*NBAS(4)*NTUVABCDFL
     &                       + (IMAP(N)-1)*NTUVABCDFL + IRABCD
              GABR11(N,IAB) = GABR11(N,IAB)
     &                         + DREAL(ECD11(IMAP(N),ICD))*RCTTFL(IAD)
            ENDDO
          ENDIF
421       CONTINUE
C
C         CONTRIBUTIONS TO Im{G(AB|--)} FROM EACH Im{E(CD|--)} ADDRESS
     ...
     &                         + DIMAG(ECD11(IMAP(N),ICD))*RCTTFL(IAD)
            ENDDO
          ENDIF
422       CONTINUE
C
C         CONTRIBUTIONS TO Re{G(AB|+-)} FROM EACH Re{E(CD|+-)} ADDRESS
     ...
     &                         + DREAL(ECD21(IMAP(N),ICD))*RCTTFL(IAD)
            ENDDO
          ENDIF
423       CONTINUE
C
C         CONTRIBUTIONS TO Im{G(AB|+-)} FROM EACH Im{E(CD|+-)} ADDRESS
     ...
     &                         + DIMAG(ECD21(IMAP(N),ICD))*RCTTFL(IAD)
            ENDDO
          ENDIF
424       CONTINUE
C
