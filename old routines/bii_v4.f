...
200   CONTINUE
      ...
      TBEC = TBEC+T2-T1
C
c      write(*,*) 'g(ab|cd) gen',l0case,rcfile
C**********************************************************************C
...
c      IF(ITOG.NE.0) THEN
c        IF(.NOT.RCFILE.OR.L0CASE) THEN
c          DO N=1,MAXN
c            PQ(N,1) = PQ(IMAP(N),1)
c            PQ(N,2) = PQ(IMAP(N),2)
c            PQ(N,3) = PQ(IMAP(N),3)
c            APH(N)  = APH(IMAP(N))
c          ENDDO
c        ENDIF
c      ENDIF
...
c      IF(RCFILE.AND..NOT.L0CASE) THEN
        MBCH = MAXCD
c      ELSE
c        MBCH = MAXN
c      ENDIF
