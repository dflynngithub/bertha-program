c     this version plots a map of the error with variations
c     in beta and gamma
...
C
     ...
     &          XB(0:NUP),Y2B(0:NUP),D2B(0:NUP)
      ...
      DIMENSION BESTEPS(0:NN,0:NN),BESTALF(0:NN,0:NN)
C
...
C     IMPORT SPLINE DATA FOR X*CHI_1(X) GIVEN 0 <= X <= XSPL
      INQUIRE(FILE='chi1x.dat',EXIST=FILETHERE)
...
C     IMPORT SPLINE DATA FOR CHI_2(X) GIVEN 0 <= X <= XSPL
      INQUIRE(FILE='chi2_small.dat',EXIST=FILETHERE)
      IF(.NOT.FILETHERE) CALL CHIGEN
...
C     IMPORT SPLINE DATA FOR CHI_2(X) GIVEN X >= XSPL
      INQUIRE(FILE='chi2_big.dat',EXIST=FILETHERE)
      IF(.NOT.FILETHERE) CALL CHIGEN
...
C     GAMA = 0.21D+0
      DO IB=0,NN
        BETA = 1.55D+0 + 0.10D+0*IB/DFLOAT(NN)
        DO IC=0,NN
          GAMA = 0.15D+0 + 0.10D+0*IC/DFLOAT(NN)
C      BETA = 1.58D+0
C      GAMA = 0.22D+0
C
...
C     BEST-FIT VALUES FOR ALPHA, BETA AND GAMMA
      ...
      BESTEPS(IB,IC) = 1.0D0-RSQBIG
      BESTALF(IB,IC) = AF
      
      WRITE(*,*) AF,BF,GF,RSQBIG
      ENDDO
      ENDDO
C
...
C
C     OPEN DATA FILE
      OPEN(UNIT=8,FILE='plots/BESTEPS.dat',STATUS='REPLACE')
C
C       BEGIN LOOP OVER ALL POINTS
        DO IB=0,NN
C
C         SAVE MATRIX ELEMENTS
          WRITE(8,*) (BESTEPS(IB,IC),IC=0,NN)
C
        ENDDO
C
C     CLOSE DATA FILE
...
C
C     OPEN DATA FILE
      OPEN(UNIT=8,FILE='plots/BESTALF.dat',STATUS='REPLACE')
C
C       BEGIN LOOP OVER ALL POINTS
        DO IB=0,NN
C
C         SAVE MATRIX ELEMENTS
          WRITE(8,*) (BESTALF(IB,IC),IC=0,NN)
C
        ENDDO
C
C     CLOSE DATA FILE
      CLOSE(UNIT=8)
      CALL GNUDENS(XOUT,TITLE,NN,N)
