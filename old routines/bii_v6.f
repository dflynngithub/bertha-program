...
C**********************************************************************C
C
C     LOOP OVER CARTESIAN INDEX ICMP FOR CENTRE CD (USE INDEX 6000)
      DO 6000 ICMP=1,3
C
...
C           CONTRIBUTIONS TO Re{G(CD|--)} FROM EACH Re{E(AB|--)} ADDRESS
            IF(ICDR11(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 411
            IF(IABR11(IJ,IAB,ICMP).EQ.0) GOTO 411
            DO N=1,MAXN
              GCDR11(N,ICD) = GCDR11(N,ICD)
     &                  + DREAL(EAB11(IMAP(N),IAB,ICMP))*RC(N,IRABCD)
            ENDDO
411         CONTINUE
C
C           CONTRIBUTIONS TO Im{G(CD|--)} FROM EACH Im{E(AB|--)} ADDRESS
            IF(ICDI11(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 412
            IF(IABI11(IJ,IAB,ICMP).EQ.0) GOTO 412
            DO N=1,MAXN
              GCDI11(N,ICD) = GCDI11(N,ICD)
     &                  + DIMAG(EAB11(IMAP(N),IAB,ICMP))*RC(N,IRABCD)
            ENDDO
412         CONTINUE
C
C           CONTRIBUTIONS TO Re{G(CD|+-)} FROM EACH Re{E(AB|+-)} ADDRESS
            IF(ICDR21(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 413
            IF(IABR21(IJ,IAB,ICMP).EQ.0) GOTO 413
            DO N=1,MAXN
              GCDR21(N,ICD) = GCDR21(N,ICD)
     &                  + DREAL(EAB21(IMAP(N),IAB,ICMP))*RC(N,IRABCD)
            ENDDO
413         CONTINUE
C
C           CONTRIBUTIONS TO Im{G(CD|+-)} FROM EACH Im{E(AB|+-)} ADDRESS
            IF(ICDI21(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 414
            IF(IABI21(IJ,IAB,ICMP).EQ.0) GOTO 414
            DO N=1,MAXN
              GCDI21(N,ICD) = GCDI21(N,ICD)
     &                  + DIMAG(EAB21(IMAP(N),IAB,ICMP))*RC(N,IRABCD)
            ENDDO
414         CONTINUE
C
...
C
C           CONTRIBUTIONS TO Re{G(CD|--)} FROM EACH Re{E(AB|--)} ADDRESS
            IF(ICDR11(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 451
            IF(IABR11(IJ,IAB,ICMP).EQ.0) GOTO 451
            DO N=1,MAXN
              GCDR11(N,ICD) = GCDR11(N,ICD)
     &           + DREAL(EAB11(IMAP(N),IAB,ICMP))*RCTTFL(IAD+IMAP(N))
            ENDDO
451         CONTINUE
C
C           CONTRIBUTIONS TO Im{G(CD|--)} FROM EACH Im{E(AB|--)} ADDRESS
            IF(ICDI11(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 452
            IF(IABI11(IJ,IAB,ICMP).EQ.0) GOTO 452
            DO N=1,MAXN
              GCDI11(N,ICD) = GCDI11(N,ICD)
     &           + DIMAG(EAB11(IMAP(N),IAB,ICMP))*RCTTFL(IAD+IMAP(N))
            ENDDO
452         CONTINUE
C
C           CONTRIBUTIONS TO Re{G(CD|+-)} FROM EACH Re{E(AB|+-)} ADDRESS
            IF(ICDR21(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 453
            IF(IABR21(IJ,IAB,ICMP).EQ.0) GOTO 453
            DO N=1,MAXN
              GCDR21(N,ICD) = GCDR21(N,ICD)
     &           + DREAL(EAB21(IMAP(N),IAB,ICMP))*RCTTFL(IAD+IMAP(N))
            ENDDO
453         CONTINUE
C
C           CONTRIBUTIONS TO Im{G(CD|+-)} FROM EACH Im{E(AB|+-)} ADDRESS
            IF(ICDI21(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 454
            IF(IABI21(IJ,IAB,ICMP).EQ.0) GOTO 454
            DO N=1,MAXN
              GCDI21(N,ICD) = GCDI21(N,ICD)
     &           + DIMAG(EAB21(IMAP(N),IAB,ICMP))*RCTTFL(IAD+IMAP(N))
            ENDDO
454         CONTINUE
C
C           SKIP POINT FOR RC(AB|CD) AND E(AB) SCREENING
...
C >>>>>   GAUGE TERM (REQUIRES ADDITIONAL CARTESIAN SUM Q')
          IF(BGAUNT) GOTO 480
C
C         LOOP OVER CARTESIAN INDEX JCMP FOR CENTRE AB
          DO JCMP=1,3
C
C           SKIP THIS STEP IF THE E(AB) FAILS SCREENING CONDITION
            IF(IABR11(IJ,IAB,JCMP)+IABI11(IJ,IAB,JCMP)
     &        +IABR21(IJ,IAB,JCMP)+IABI21(IJ,IAB,JCMP).EQ.0) THEN
              GOTO 403
            ENDIF
C
...
C
C           CONTRIBUTIONS TO Re{G(CD|--)} FROM EACH Re{E(AB|--)} ADDRESS
            IF(ICDR11(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 415
            IF(IABR11(IJ,IAB,JCMP).EQ.0) GOTO 415
            DO N=1,MAXN
              GCDR11(N,ICD) = GCDR11(N,ICD)
     &                          - DREAL(EAB11(IMAP(N),IAB,JCMP))*T(N)
            ENDDO
415         CONTINUE
C
C           CONTRIBUTIONS TO Im{G(CD|--)} FROM EACH Im{E(AB|--)} ADDRESS
            IF(ICDI11(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 416
            IF(IABI11(IJ,IAB,JCMP).EQ.0) GOTO 416
            DO N=1,MAXN
              GCDI11(N,ICD) = GCDI11(N,ICD)
     &                          - DIMAG(EAB11(IMAP(N),IAB,JCMP))*T(N)
            ENDDO
416         CONTINUE
C
C           CONTRIBUTIONS TO Re{G(CD|--)} FROM EACH Re{E(AB|--)} ADDRESS
            IF(ICDR21(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 417
            IF(IABR21(IJ,IAB,JCMP).EQ.0) GOTO 417
            DO N=1,MAXN
              GCDR21(N,ICD) = GCDR21(N,ICD)
     &                          - DREAL(EAB21(IMAP(N),IAB,JCMP))*T(N)
            ENDDO
417         CONTINUE
C
C           CONTRIBUTIONS TO Im{G(CD|--)} FROM EACH Im{E(AB|--)} ADDRESS
            IF(ICDI21(ICD,ICMP).EQ.0.AND.ISYM.GE.1) GOTO 418
            IF(IABI21(IJ,IAB,JCMP).EQ.0) GOTO 418
            DO N=1,MAXN
              GCDI21(N,ICD) = GCDI21(N,ICD)
     &                          - DIMAG(EAB21(IMAP(N),IAB,JCMP))*T(N)
            ENDDO
418         CONTINUE
C
...
403         CONTINUE
C
C           END LOOP OVER CARTESIAN INDEX JCMP FOR CENTRE AB
            ENDDO
C
...
C     RAW CONTRACTION (--|--) = E(CD|--)*(Re{G(CD|--)} + i*Im{G(CD|--)})
      DO ICD=1,NTUVCD
        IF(ICDR11(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR1(N) = QR1(N) + DREAL(ECD11(IJ,ICD,ICMP))*GCDR11(N,ICD)
          ENDDO
        ENDIF
        IF(ICDI11(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR2(N) = QR2(N) - DIMAG(ECD11(IJ,ICD,ICMP))*GCDI11(N,ICD)
          ENDDO
        ENDIF
      ENDDO
C
...
C     RAW CONTRACTION (+-|+-) = E(CD|+-)*(Re{G(CD|+-)} + i*Im{G(CD|+-)})
      DO ICD=1,NTUVCD
        IF(ICDR21(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR1(N) = QR1(N) + DREAL(ECD21(IJ,ICD,ICMP))*GCDR21(N,ICD)
          ENDDO
        ENDIF
        IF(ICDI21(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR2(N) = QR2(N) - DIMAG(ECD21(IJ,ICD,ICMP))*GCDI21(N,ICD)
          ENDDO
        ENDIF
      ENDDO
C
...
C     RAW CONTRACTION (--|--) = E(CD|--)*(Re{G(CD|--)} + i*Im{G(CD|--)})
      DO ICD=1,NTUVCD
        IF(ICDR11(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR1(N) = QR1(N) + DREAL(ECD11(IJ,ICD,ICMP))*GCDR11(N,ICD)
            QI2(N) = QI2(N) + DREAL(ECD11(IJ,ICD,ICMP))*GCDI11(N,ICD)
          ENDDO
        ENDIF
        IF(ICDI11(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QI1(N) = QI1(N) + DIMAG(ECD11(IJ,ICD,ICMP))*GCDR11(N,ICD)
            QR2(N) = QR2(N) - DIMAG(ECD11(IJ,ICD,ICMP))*GCDI11(N,ICD)
          ENDDO
        ENDIF
      ENDDO
...
C     RAW CONTRACTION (--|+-) = E(CD|--)*(Re{G(CD|+-)} + i*Im{G(CD|+-)})
      DO ICD=1,NTUVCD
        IF(ICDR11(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR1(N) = QR1(N) + DREAL(ECD11(IJ,ICD,ICMP))*GCDR21(N,ICD)
            QI2(N) = QI2(N) + DREAL(ECD11(IJ,ICD,ICMP))*GCDI21(N,ICD)
          ENDDO
        ENDIF
        IF(ICDI11(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QI1(N) = QI1(N) + DIMAG(ECD11(IJ,ICD,ICMP))*GCDR21(N,ICD)
            QR2(N) = QR2(N) - DIMAG(ECD11(IJ,ICD,ICMP))*GCDI21(N,ICD)
          ENDDO
        ENDIF
      ENDDO
C
...
C     RAW CONTRACTION (+-|--) = E(CD|+-)*(Re{G(CD|--)} + i*Im{G(CD|--)})
      DO ICD=1,NTUVCD
        IF(ICDR21(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR1(N) = QR1(N) + DREAL(ECD21(IJ,ICD,ICMP))*GCDR11(N,ICD)
            QI2(N) = QI2(N) + DREAL(ECD21(IJ,ICD,ICMP))*GCDI11(N,ICD)
          ENDDO
        ENDIF
        IF(ICDI21(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QI1(N) = QI1(N) + DIMAG(ECD21(IJ,ICD,ICMP))*GCDR11(N,ICD)
            QR2(N) = QR2(N) - DIMAG(ECD21(IJ,ICD,ICMP))*GCDI11(N,ICD)
          ENDDO
        ENDIF
      ENDDO
C
...
C     RAW CONTRACTION (+-|+-) = E(CD|+-)*(Re{G(CD|+-)} + i*Im{G(CD|+-)})
      DO ICD=1,NTUVCD
        IF(ICDR21(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QR1(N) = QR1(N) + DREAL(ECD21(IJ,ICD,ICMP))*GCDR21(N,ICD)
            QI2(N) = QI2(N) + DREAL(ECD21(IJ,ICD,ICMP))*GCDI21(N,ICD)
          ENDDO
        ENDIF
        IF(ICDI21(ICD,ICMP).NE.0) THEN
          DO N=1,MAXN
            QI1(N) = QI1(N) + DIMAG(ECD21(IJ,ICD,ICMP))*GCDR21(N,ICD)
            QR2(N) = QR2(N) - DIMAG(ECD21(IJ,ICD,ICMP))*GCDI21(N,ICD)
          ENDDO
        ENDIF
      ENDDO
