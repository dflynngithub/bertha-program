      SUBROUTINE ERI(RR,XYZ,KQN,MQN,EXPT,NBAS,ITN,IBAS,JBAS)
...
C       READ RC(AB|CD) INTEGRALS FROM THIS STARTING POINT
        ...
        DO KBAS=1,NBAS(3)
          DO LBAS=1,NBAS(4)
            M = M+1
            DO IABCD=1,NTUVABCD
              IAD = IADRTT + (M-1)*NTUVABCDFL + IABCD
...
C       GENERATE R-INTEGRALS
        CALL RMAKE(RC,PQ,APH,MAXCD,LAMABCDFL)
C
...
C         COPY THIS BATCH OF INTEGRALS TO A SAVED LIST
          ...
          DO KBAS=1,NBAS(3)
            DO LBAS=1,NBAS(4)
              M = M+1
              DO IABCDFL=1,NTUVABCDFL
                IAD = IADRTT + (M-1)*NTUVABCDFL + IABCDFL
                RCTTFL(IAD)  = RC(M,IABCDFL)
              ENDDO
            ENDDO
          ENDDO
C
...
C         CONTRIBUTIONS TO Re{G(AB|--)} FROM EACH Re{E(CD|--)} ADDRESS
          ...
          IF(ICDR11(ICD).EQ.1) THEN
            DO M=1,MAXCD
              GABR11(M,IAB) = GABR11(M,IAB)
     &                               + DREAL(ECD11(M,ICD))*RC(M,IRABCD)
            ENDDO
          ENDIF
411       CONTINUE
C
C         CONTRIBUTIONS TO Im{G(AB|--)} FROM EACH Im{E(CD|--)} ADDRESS
          ...
          IF(ICDI11(ICD).EQ.1) THEN
            DO M=1,MAXCD
              GABI11(M,IAB) = GABI11(M,IAB)
     &                               + DIMAG(ECD11(M,ICD))*RC(M,IRABCD)
            ENDDO
          ENDIF
412       CONTINUE
C
C         CONTRIBUTIONS TO Re{G(AB|+-)} FROM EACH Re{E(CD|+-)} ADDRESS
          ...
          IF(ICDR21(ICD).EQ.1) THEN
            DO M=1,MAXCD
              GABR21(M,IAB) = GABR21(M,IAB)
     &                               + DREAL(ECD21(M,ICD))*RC(M,IRABCD)
            ENDDO
          ENDIF
413       CONTINUE
C
C         CONTRIBUTIONS TO Im{G(AB|+-)} FROM EACH Im{E(CD|+-)} ADDRESS
          ...
          IF(ICDI21(ICD).EQ.1) THEN
            DO M=1,MAXCD
              GABI21(M,IAB) = GABI21(M,IAB)
     &                               + DIMAG(ECD21(M,ICD))*RC(M,IRABCD)
            ENDDO
          ENDIF
414       CONTINUE
C
...
C     RAW CONTRACTION (--|--) = E(AB|--)*(Re{G(AB|--)} + i*Im{G(AB|--)})
      DO IAB=1,NTUVAB
        IF(IABR11(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QR1(M) = QR1(M) + DREAL(EAB11(IJ,IAB))*GABR11(M,IAB)
            QI2(M) = QI2(M) + DREAL(EAB11(IJ,IAB))*GABI11(M,IAB)
          ENDDO
        ENDIF
        IF(IABI11(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QI1(M) = QI1(M) + DIMAG(EAB11(IJ,IAB))*GABR11(M,IAB)
            QR2(M) = QR2(M) - DIMAG(EAB11(IJ,IAB))*GABI11(M,IAB)
          ENDDO
        ENDIF
      ENDDO
C
C     APPLY PHASE RELATIONS AND NORMALISATION FACTORS TO RAW CONTRACTION
      DO M=1,MAXCD
        RR(M, 1) =     PRE(M)*DCMPLX(QR1(M)+QR2(M),QI1(M)+QI2(M))
        RR(M, 4) = PCD*PRE(M)*DCMPLX(QR1(M)-QR2(M),QI1(M)-QI2(M))
...
C     RAW CONTRACTION (--|+-) = E(AB|--)*(Re{G(AB|+-)} + i*Im{G(AB|+-)})
      DO IAB=1,NTUVAB
        IF(IABR11(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QR1(M) = QR1(M) + DREAL(EAB11(IJ,IAB))*GABR21(M,IAB)
            QI2(M) = QI2(M) + DREAL(EAB11(IJ,IAB))*GABI21(M,IAB)
          ENDDO
        ENDIF
        IF(IABI11(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QI1(M) = QI1(M) + DIMAG(EAB11(IJ,IAB))*GABR21(M,IAB)
            QR2(M) = QR2(M) - DIMAG(EAB11(IJ,IAB))*GABI21(M,IAB)
          ENDDO
        ENDIF
      ENDDO
C
C     APPLY PHASE RELATIONS AND NORMALISATION FACTORS TO RAW CONTRACTION
      DO M=1,MAXCD
        RR(M, 3) =     PRE(M)*DCMPLX(QR1(M)+QR2(M),QI1(M)+QI2(M))
        RR(M, 2) =-PCD*PRE(M)*DCMPLX(QR1(M)-QR2(M),QI1(M)-QI2(M))
...
C     RAW CONTRACTION (+-|--) = E(AB|+-)*(Re{G(AB|--)} + i*Im{G(AB|--)})
      DO IAB=1,NTUVAB
        IF(IABR21(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QR1(M) = QR1(M) + DREAL(EAB21(IJ,IAB))*GABR11(M,IAB)
            QI2(M) = QI2(M) + DREAL(EAB21(IJ,IAB))*GABI11(M,IAB)
          ENDDO
        ENDIF
        IF(IABI21(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QI1(M) = QI1(M) + DIMAG(EAB21(IJ,IAB))*GABR11(M,IAB)
            QR2(M) = QR2(M) - DIMAG(EAB21(IJ,IAB))*GABI11(M,IAB)
          ENDDO
        ENDIF
      ENDDO
C
C     APPLY PHASE RELATIONS AND NORMALISATION FACTORS TO RAW CONTRACTION
      DO M=1,MAXCD
        RR(M, 9) =     PRE(M)*DCMPLX(QR1(M)+QR2(M),QI1(M)+QI2(M))
        RR(M,12) = PCD*PRE(M)*DCMPLX(QR1(M)-QR2(M),QI1(M)-QI2(M))
...
C     RAW CONTRACTION (+-|+-) = E(AB|+-)*(Re{G(AB|+-)} + i*Im{G(AB|+-)})
      DO IAB=1,NTUVAB
        IF(IABR21(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QR1(M) = QR1(M) + DREAL(EAB21(IJ,IAB))*GABR21(M,IAB)
            QI2(M) = QI2(M) + DREAL(EAB21(IJ,IAB))*GABI21(M,IAB)
          ENDDO
        ENDIF
        IF(IABI21(IAB).EQ.1) THEN
          DO M=1,MAXCD
            QI1(M) = QI1(M) + DIMAG(EAB21(IJ,IAB))*GABR21(M,IAB)
            QR2(M) = QR2(M) - DIMAG(EAB21(IJ,IAB))*GABI21(M,IAB)
          ENDDO
        ENDIF
      ENDDO
C
C     APPLY PHASE RELATIONS AND NORMALISATION FACTORS TO RAW CONTRACTION
      DO M=1,MAXCD
        RR(M,11) =     PRE(M)*DCMPLX(QR1(M)+QR2(M),QI1(M)+QI2(M))
        RR(M,10) =-PCD*PRE(M)*DCMPLX(QR1(M)-QR2(M),QI1(M)-QI2(M))
